...
public class MyAdapterForFriendProfilePost extends RecyclerView.Adapter<MyAdapterForFriendProfilePost.ViewHolder> {
    ...
    public MyAdapterForFriendProfilePost(String fuid,ArrayList<DataSnapshot> dataSnapshots)
    {
    ...
    public void onBindViewHolder(@NonNull final ViewHolder holder, int positionn) {
        ...
        mRef.child("users").child(fuid).addValueEventListener(new ValueEventListener() {
            @Override
        ...
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                AppCompatActivity activity = (AppCompatActivity) view.getContext();
                Fragment fragment = new ViewSinglePost(image_key,fuid);
                ...
                mRef.child("likes").child(image_key).child(uid).addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if(dataSnapshot.exists())
                        {
                            ...
                            mRef.child("Notification").child(fuid).child(image_key+uid).child("status").setValue("0");
                            mRef.child("Notification").child(fuid).child(image_key+uid).child("type").setValue("1"); //1 for like
                            mRef.child("Notification").child(fuid).child(image_key+uid).child("from").setValue(uid);
                            mRef.child("Notification").child(fuid).child(image_key+uid).child("post_id").setValue(image_key);
                            long timeMillis = System.currentTimeMillis();
                            mRef.child("Notification").child(fuid).child(image_key+uid).child("timestamp").setValue(timeMillis);
                        }
                    }
        ...
        holder.submit_comment_button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(comment.length()>0) {
                    ...
                    mRef.child("Notification").child(fuid).child(some_key).child("status").setValue("0");
                    mRef.child("Notification").child(fuid).child(some_key).child("type").setValue("2"); //2 for comment
                    mRef.child("Notification").child(fuid).child(some_key).child("post_id").setValue(image_key);
                    mRef.child("Notification").child(fuid).child(some_key).child("from").setValue(uid);
                    mRef.child("Notification").child(fuid).child(some_key).child("timestamp").setValue(timeMillis);
                }
            }
        });
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        TextView count_like,count_comment;
